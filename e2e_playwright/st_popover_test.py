...
def test_that_popover_does_not_auto_focus(app: Page):
    """Test that the popover does not auto focus."""
    popover_container = open_popover(app, "popover 9 (auto focus)")
    # The date input element should not be focused:
    expect(popover_container.get_by_test_id("stDateInput")).not_to_be_focused()
    # Check that the date input calendar is not attached:
    expect(popover_container.locator("[data-baseweb='calendar']")).not_to_be_attached()
